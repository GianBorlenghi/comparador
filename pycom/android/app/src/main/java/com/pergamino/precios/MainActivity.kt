...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val chipsBox = findViewById<ChipGroup>(R.id.chipsBox) ?: findViewById<View>(R.id.chipsBox) as? ChipGroup

        // Chips
        val chipContainer = findViewById<View>(R.id.chipsBox)
        if(chipContainer is ChipGroup){
        ...
        adapter = ProdAdapter(emptyList()) { url ->
            try{ startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(url))) }catch{}
        }
                ...
                try{
                    ...
                    for(j in 0 until items.length()){
                        ...
                        for(k in 0 until sellers.length()){
                            ...
                            val pf=of.optDouble("Price",Double.NaN); if(pf.isNaN()||pf<500) continue
                    ...
                    if(promos.isEmpty() && po>pf && po/pf<=3){
                    ...
                    for(p in promos){
                        ...
                        if(m1!=null){ val pct=m1.groupValues[1].toInt(); if(pct in 1..90){ val tot=pf+pf*(1-pct/100.0); eff=formatear(tot/2) } }
                        else if(low.contains("3x2")) eff=formatear(pf*2/3)
                        else if(low.contains("2x1")) eff=formatear(pf/2)
                        if(eff!=null){ promoEff="$p → $eff c/u llevando ${if(p.contains("3x2"))"3" else "2"}"; break }
                    }
                    val promosFinal=if(promoEff!=null) listOf(promoEff) else promos.take(2)
                    out.add(Producto(superN,nombre,pf,po,formatear(pf),promosFinal,null,promoEff,imagen,link))
                    if(out.size>=8) break
                }catch{}
            }
